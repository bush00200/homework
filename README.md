...
- ex07: [ex07/ex07.ino](ex07/ex07.ino) —— Web 网页端无极调光器（滑动条 + PWM），拖动滑块实时平滑调节 LED 亮度
- ex08: [ex08/ex08.ino](ex08/ex08.ino) —— 物联网安防报警器模拟（布防/撤防 + 触摸触发报警锁定），撤防后复位
- ex09: [ex09/ex09.ino](ex09/ex09.ino) —— 实时传感器 Web 仪表盘（触摸数值上报 + AJAX 刷新），网页显示实时跳动数值
...
- 程序文件：[ex07/ex07.ino](ex07/ex07.ino)
- 核心：网页端滑动条（0~255）通过 `fetch()` 发送到 ESP32，ESP32 解析参数并用 `ledcWrite()` 实时调整 PWM 占空比
- 预期效果：浏览器拖动滑块时 LED 亮度实时、平滑变化

### ex08：物联网安防报警器模拟（布防/撤防 + 报警锁定）

- 程序文件：[ex08/ex08.ino](ex08/ex08.ino)
- 核心：网页按钮布防/撤防；布防后触摸引脚触发报警并锁定高频闪烁，直到撤防才停止并复位
- 预期效果：未布防触摸无反应；布防后触发即报警锁定；仅网页撤防可解除

### ex09：实时传感器 Web 仪表盘（触摸数值上报 + AJAX）

- 程序文件：[ex09/ex09.ino](ex09/ex09.ino)
- 核心：ESP32 提供数据接口返回 touchRead 原始值；网页端 AJAX 定时拉取并居中显示实时数值
- 预期效果：手靠近引脚数值实时变小，离开后恢复
